...
contract Campaign {
    ...
    struct Request {
        ...
        uint256 value; // Amount of money in Wei that the manager wants to send to the vendor
        address payable recipient; // Address that the money will be sent to. See this a the vendor
    ...
    constructor(uint256 minimum, address creator_address) {
    ...
    function finalizeRequest(uint64 request_index) public restricted{
        ...
        // vendor = payable(request.recipient);
        // vendor.transfer(request.value);
        // Use the .call() method instead of the deprecated .transfer()
